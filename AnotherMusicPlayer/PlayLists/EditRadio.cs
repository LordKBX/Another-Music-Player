...
{
    public partial class EditRadio : Form
    {
        public WebRadioItem originItem = null;
        private string LogoInfo = null;
        private Bitmap image = null;

        public EditRadio(Form parent, WebRadioItem item = null, int category = 0)
        {
            InitializeComponent();
            ...
            Text = TitleLabel.Text = App.GetTranslation((item == null) ? "PlaylistsWindowEditRadioAddWindowLabel" : "PlaylistsWindowEditRadioEditWindowLabel");

            ModeLabel.Text = App.GetTranslation("PlaylistsWindowEditRadioAddModeLabel");
            ValidateButton.Text = App.GetTranslation("PlaylistsWindowEditRadioAddSaveButton");

            CategoryLabel.Text = App.GetTranslation("PlaylistsWindowEditRadioAddCategoryLabel");
            NameLabel.Text = App.GetTranslation("PlaylistsWindowEditRadioAddNameLabel");
            UrlLabel.Text = App.GetTranslation("PlaylistsWindowEditRadioAddUrlLabel");
            UrlPrefixLabel.Text = App.GetTranslation("PlaylistsWindowEditRadioAddUrlPrefixLabel");

            DescriptionLabel.Text = App.GetTranslation("PlaylistsWindowEditRadioAddDescriptionLabel");

            NameTextBox.Text = (item != null) ? item.Name : "";
            UrlTextBox.Text = (item != null) ? item.Url : "";
            UrlPrefixTextBox.Text = (item != null) ? item.UrlPrefix : "";
            DescriptionRichTextBox.Text = (item != null) ? item.Description : "";

            CategoryComboBox.Items.Add(new DropDownItem() { Value = "- - - - - - - ", Data = 0 });
            ...
            foreach (KeyValuePair<string, Dictionary<string, object>> row in rez3)
            {
                ...
                CategoryComboBox.Items.Add(new DropDownItem() { Value = "" + row.Value["Name"], Data = CRID });
                if (category != 0 && category == CRID) { CategoryComboBox.SelectedIndex = CategoryComboBox.Items.Count - 1; }
                else if (item != null && item.Category == CRID) { CategoryComboBox.SelectedIndex = CategoryComboBox.Items.Count - 1; }
            }

            if (item != null)
            {
                if (item.FType == "Stream") { ModeBoolPresenter.Value = true; } else { ModeBoolPresenter.Value = false; }
        ...
        private void LogoButtonAction_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                catch (Exception)
                {
                    ...
                    LogoButton.BackgroundImageLayout = ImageLayout.Center;
                }
            }
        }
        ...
        private void ValidateButton_Click(object sender, System.EventArgs e)
        {
            ...
            string urlprefix = UrlPrefixTextBox.Text.Trim();
            string description = DescriptionRichTextBox.Text.Trim();
            int Category = (CategoryComboBox.SelectedItem == null) ? 0 : (int)((DropDownItem)CategoryComboBox.SelectedItem).Data;
            if (name.Length < 3)
            {
            ...
            {
                int index = 1;
                Dictionary<string, Dictionary<string, object>> rez2 = App.bdd.DatabaseQuery("SELECT RID FROM radios ORDER BY CRID DESC LIMIT 1", "RID");
                ...
                originItem = new WebRadioItem() { RID = index, Name = name, 
                    Description = description, Logo = LogoInfo,
                    FType = (ModeBoolPresenter.Value)? "Stream" : "M3u"
                };
                ...
                App.bdd.DatabaseQuerys(new string[] {
                    ...
                    ((LogoInfo == null)?"NULL":"'" + Database.EscapeString(LogoInfo) + "'") + "," +
                    "'" + Database.EscapeString(url) + "'," +
                    "'" + Database.EscapeString(urlprefix) + "'," +
                    "'" + ((ModeBoolPresenter.Value) ? "Stream" : "M3u") + "'," +
                    "0," + Category + ")"
                }, true);
            }
            else
            {
                App.bdd.DatabaseQuerys(new string[] {
                    ...
                    "Logo = " + ((LogoInfo == null)?"NULL":"'" + Database.EscapeString(LogoInfo) + "'") + "," +
                    "Url = '" + Database.EscapeString(url) + "'," +
                    "UrlPrefix = '" + Database.EscapeString(urlprefix) + "'," +
                    "FType = '" + ((ModeBoolPresenter.Value) ? "Stream" : "M3u") + "'," +
                    "Fragmented = 0, Category = " + Category + " WHERE RID = " + originItem.RID
                }, true);
            }
